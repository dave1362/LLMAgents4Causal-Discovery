...
from algs.dagma_baseline.dagma_wrapper import (
    dagma_nonlinear_wrapper,
    dagma_linear_wrapper,
)
import numpy as np
...
if args.dataset == "neuropathic":
    ...
    data = df.to_numpy()
elif args.dataset in ["Auto_MPG", "DWD_climate", "Sachs", "Product_Review", "Cloud_Computing"]:
    df = pd.read_csv(f"./data/{args.dataset}_data.csv")
...
if args.alg == "ges":
    ...
    predicted_adj_mat = ges(
        data, score_func="local_score_BIC", maxP=None, parameters=None
    )
elif args.alg == "pc":
    data = data + gaussian_noise
    predicted_adj_mat = pc(data)
elif args.alg == "dagma_nonlinear":
    predicted_adj_mat = dagma_nonlinear_wrapper(
        data, len(adj_mat), args.lambda1, args.lambda2
    )
elif args.alg == "dagma_linear":
    predicted_adj_mat = dagma_linear_wrapper(data, args.lambda1)
elif args.alg == "notears":
    predicted_adj_mat = notears(
        data, lambda1=args.lambda1, loss_type="l2", w_threshold=args.w_threshold
    )
elif args.alg == "llm_pairwise":
    predicted_adj_mat = llm_pairwise(
        VAR_NAMES_AND_DESC[args.dataset],
        prompts[args.dataset],
        df,
    ...
    predicted_adj_mat = llm_bfs(
        VAR_NAMES_AND_DESC[args.dataset], args.dataset, df, include_statistics=False
    )
elif args.alg == "llm_bfs_with_statistics":
    predicted_adj_mat = llm_bfs(
        VAR_NAMES_AND_DESC[args.dataset], args.dataset, df, include_statistics=True
    )
else:
    raise ValueError(f"Unknown algorithm {args.alg}")


print(type(predicted_adj_mat))
df = pd.DataFrame(predicted_adj_mat)
df.to_csv(
    f"./causal_LLM_Matrix/{args.dataset}_adjacency_matrix.csv",
    index=False,
    header=False,
)


# Save predicted adjacency matrix
